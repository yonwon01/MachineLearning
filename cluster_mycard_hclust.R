...
# skew 분포모양 의미하는 것(다른 한쪽으로 치우쳐 있는지 확인하는 것) :왜도
# kurtosis( 분포모양이 뾰족한지 확인하는 것):첨도 
...
## 박스플롯에서 이상치 확인방법 
x <- boxplot(raw$variety) #stats:안정치 라인 수 와 값   out: outlier(결측치) 
str(x)
x$out
...
dist(raw) # 관찰치간 거리계산
d <- dist(raw)
as.matrix(d)[1:2, 1:2] # 2개 관찰치간 거리계산 값을 조회 1번부터 2번까지 거리 비교한거만 보여주는거 
...
fit.average <- hclust(d, method="average")  #d 거리 데이터로 

## 계층적 군집분석 그래프화
plot(fit.average, hang=-1, cex=.8,  #hang=-1,가지의 위치 , cex=.8글씨 크기
...
#몇개정도로 묶는게 좋을지 알아본다
nc <- NbClust(raw.scaled, distance="euclidean",  #raw.scaled:표준화된 raw데이터로 비교
              min.nc=2, max.nc=15, method="average")  #max.nc=15개가 넘어간 클러스터는 안한다란 뜻
nc # 26개 군집갯수 선정기준의 계산결과  # ex)  KL 방법으로 했을땐 12개로 나누는게 적당하다.    26개중에서 가장 많이 나누자고 한 것으로 클러스터링을 나누는게 가장 적당하다. 
...
clusters <- cutree(fit.average, k=5)    #cutree: 댄드로그램에 딸려있는 나뭇가지를 실제로 자름 
clusters # 관찰치별로 어떤 군집에 속하는지 확인
table(clusters) # 5개 군집별로 속해 있는 관찰치 갯수 확인
View(raw)

## 기존 데이터셋에 그룹표시 변수컬럼 추가
raw2 <- data.frame(raw, group=clusters)# raw 데이터 옆에 group이라는 컬럼을 만들어서 붙여라 
raw2
psych::headTail(raw2)
...
## 그룹명칭 부여  #factor:범주형 데이터를 만드는 것 ex) 남자여자  외국인내국
...
##boxplot  각변수들이 겹쳐서 안보여서 구분하기가 쉬움 
featurePlot(x = raw2[ , c(1:4)], y = raw2$group.f,  #group.f 각그룹의 이름
            plot = "box",
